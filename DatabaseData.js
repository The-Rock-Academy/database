...
class DatabaseData {
  ...
  setVariable(variableName, variableValue) {
    let textFinderResults = this.sheet.getRange(1,1, this.sheet.getMaxRows(), 2).createTextFinder(variableName).matchEntireCell(true).findAll()
    if (textFinderResults.length == 0) {
      throw new Error("Tried to find '" + variableName + "' in the data sheet in the Database. It could not be found. Make sure that it exists or that there isn't a typo somewhere.");
    } else if (textFinderResults.length > 1) {
      throw new Error("More than one occurance of '" + variableName + "' was found in the data sheet. Make sure there is only one as having multiple could cause errors")
    } else {
      this.sheet.getRange(textFinderResults[0].getRow(), 2).setValue(variableValue);
    }
  }
